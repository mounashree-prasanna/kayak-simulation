...
def _convert_datetime_in_dict(data):
    """Recursively convert datetime objects in dict/list to ISO strings"""
    if isinstance(data, dict):
        return {k: _convert_datetime_in_dict(v) for k, v in data.items()}
...
class CachingService:
    ...
    async def cache_deals(self, route: str, listing_type: str, deals: List[Dict], ttl: int = None):
        ...
        serializable_deals = _convert_datetime_in_dict(deals)
        
        await self.redis.setex(
            cache_key,
            ttl,
            json.dumps(serializable_deals)
        )
    ...
    async def cache_listing(self, listing_id: str, deal: Dict, ttl: int = None):
        ...
        # Convert datetime objects to ISO format strings before serializing
        serializable_deal = _convert_datetime_in_dict(deal)
        
        await self.redis.setex(
            cache_key,
            ttl,
            json.dumps(serializable_deal)
        )
