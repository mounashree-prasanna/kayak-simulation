...
app.get('/health', (req, res) => {
  res.status(200).json({
    ...
    services: {
      ...
      agentic_recommendation: AGENTIC_RECOMMENDATION_SERVICE_URL
    }
  });
});
...
// Proxy routes for agentic recommendation service
app.use('/api/intent', createProxyMiddleware({
  ...
  logLevel: 'info',
  onProxyReq: (proxyReq, req, res) => {
    console.log(`[API Gateway] Proxying ${req.method} ${req.originalUrl || req.url} to ${AGENTIC_RECOMMENDATION_SERVICE_URL}/intent`);
  },
  onProxyRes: (proxyRes, req, res) => {
    console.log(`[API Gateway] Received ${proxyRes.statusCode} from recommendation service for ${req.path}`);
  },
  onError: (err, req, res) => {
    console.error(`[API Gateway] Error proxying to agentic-recommendation-service: ${err.message}`);
    if (!res.headersSent) {
      res.status(502).json({
        success: false,
...
app.use('/api/bundles', createProxyMiddleware({
  ...
  logLevel: 'info',
  onProxyReq: (proxyReq, req, res) => {
    console.log(`[API Gateway] Proxying ${req.method} ${req.originalUrl || req.url} to ${AGENTIC_RECOMMENDATION_SERVICE_URL}/bundles`);
  },
  onProxyRes: (proxyRes, req, res) => {
    console.log(`[API Gateway] Received ${proxyRes.statusCode} from recommendation service for ${req.path}`);
  },
...
app.use('/api/watch', createProxyMiddleware({
  target: AGENTIC_RECOMMENDATION_SERVICE_URL,
  changeOrigin: true,
  pathRewrite: { '^/api/watch': '/watch' },
  onError: (err, req, res) => {
    console.error(`[API Gateway] Error proxying to agentic-recommendation-service: ${err.message}`);
    if (!res.headersSent) {
      res.status(502).json({
...
app.use('/api/ingest', createProxyMiddleware({
  target: AGENTIC_RECOMMENDATION_SERVICE_URL,
  changeOrigin: true,
  pathRewrite: { '^/api/ingest': '/api/ingest' },
  onError: (err, req, res) => {
    console.error(`[API Gateway] Error proxying to agentic-recommendation-service: ${err.message}`);
    if (!res.headersSent) {
      res.status(502).json({
  ...
  timeout: 60000, // Longer timeout for CSV ingestion
  proxyTimeout: 60000
}));
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`  - Agentic Recommendation: ${AGENTIC_RECOMMENDATION_SERVICE_URL}`);
    });
  } catch (error) {
